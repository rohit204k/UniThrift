...
async def get_listings_by_user(user_data: dict[str, Any], page: int, page_size: int) -> list[dict[str, Any]]:
    ...
    return await core_service.read_many(
        collection_name=Collections.LISTINGS, data_filter={'seller_id': user_data['user_id'], 'is_deleted': False}, sort={'updated_at': -1}, page=page, page_size=page_size, sort={'created_at': -1}
    )
